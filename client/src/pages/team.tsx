  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 p-3 sm:p-6 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 left-1/5 w-72 h-72 bg-gradient-to-r from-blue-400/10 to-purple-400/10 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-1/3 right-1/5 w-96 h-96 bg-gradient-to-r from-purple-400/10 to-pink-400/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
      </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm border border-white/50 rounded-2xl shadow-lg shadow-blue-500/5 p-4 sm:p-6 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 group">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="group-hover:transform group-hover:translate-x-1 transition-transform duration-200">
              <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent group-hover:from-blue-600 group-hover:to-purple-600 transition-all duration-300">
                Team Management
              </h1>
              <p className="text-slate-600 mt-1 text-sm sm:text-base opacity-80 group-hover:opacity-100 transition-opacity">Manage your team members and their assignments</p>
            </div>
            <Button 
              onClick={() => setShowAddModal(true)}
              className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 group/btn"
            >
          ...
          <div className="flex justify-center">
            <div className="relative w-full max-w-md group/search">
              ...
              <Input
                ...
                className="pl-10 bg-white/50 border-white/30 focus:bg-white/90 focus:border-blue-300 transition-all duration-200 rounded-xl hover:bg-white/70"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredMembers.map((member) => {
            ...
            return (
              <Card 
                key={member.id} 
                className="bg-white/90 backdrop-blur-sm border border-white/50 shadow-lg shadow-blue-500/5 hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 hover:scale-105 cursor-pointer group"
                onClick={() => {
                  setSelectedMember(member);
                  setShowMemberDetails(true);
                }}
              >
                <CardContent className="p-4 sm:p-6 group-hover:transform group-hover:translate-y-1 transition-transform duration-200">
                  <div className="text-center space-y-4">
                    <div className="relative">
                    <Avatar className="w-20 h-20 mx-auto border-4 border-white shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all duration-200">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="absolute -top-2 -right-2 h-8 w-8 p-0 bg-white/80 hover:bg-white shadow-md hover:shadow-lg transition-all duration-200 hover:scale-110"
                      onClick={(e) => {
                        e.stopPropagation();
